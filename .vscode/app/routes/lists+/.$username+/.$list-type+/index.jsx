<<<<<<<< HEAD:.vscode/app/routes/lists+/.$username+/.$list-type+/index.jsx
...
async function createNewList(listParams: any) {
  ...
  const emptyList = {
    ...
    header: {value: " ", type: "string"},
    typeId: {value: typeId, type: "string"},
    displayedColumns: {value: Object.keys(JSON.parse(listParams.listTypeData.columns) as Record<string, unknown>).filter(entry => entry !== "id" && entry !== "watchlistId"  && entry !== "watchlist").join(", "), type: "string"},
    createdAt: {value: Date.now(), type: "date"},
    updatedAt: {value: Date.now(), type: "date"},
    ownerId: {value: listParams.listOwner.id, type: "string"},
    description: {value: " ", type: "string"}
...
export async function loader(params: LoaderFunctionArgs) {
  ...
  for (const watchlist of watchListsSorted) {
    ...
    const entryData = {
      watchlist: watchlist,
      listEntries: listEntries.sort((a: any, b: any) => a.position - b.position)
    }
...
>>>>>>>> develop:app/routes/lists+/.$username+/.$list-type+/index.tsx
