	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<div className="cursor-pointer rounded bg-[#6F6F6F] px-[0.5rem] py-[0.1rem] text-base font-bold hover:bg-[#8CA99D]">
			...
			<DropdownMenuPortal>
				<DropdownMenuContent sideOffset={8} align="start">
					<DropdownMenuItem
						onClick={() => {
							params.columnParams.setSelectedSearchType('Movie')
						}}
					>
						Movie
					</DropdownMenuItem>
					<DropdownMenuItem
						onClick={() => {
							params.columnParams.setSelectedSearchType('TV Series')
						}}
					>
						TV Series
					</DropdownMenuItem>
			...
			onSubmit={async event => {
				...
				try {
					...
					if (params.columnParams.listTypeData.name == 'liveaction') {
						...
						results = await searchMAL(searchQuery, 'anime', 5)
					} else if (params.columnParams.listTypeData.name == 'manga') {
						results = await searchMAL(searchQuery, 'manga', 5)
					}
		...
		>
			{showDropdown ? (
				<div>
					<div className="flex flex-row flex-wrap gap-2">
						<input
							...
							placeholder="Search"
							autoComplete="off"
							className="watchlist-search-bar w-full"
							onChange={e => {
								setSearchQuery(e.target.value)
							}}
						/>
						<StatusButton type="submit" status="idle">
							<Icon name="magnifying-glass" size="md" />
						</StatusButton>
						{searchQuery && searchQuery.length < 3 ? (
							<em className="watchlist-search-error-message">
						...
						<button
							...
							onClick={async () => {
								...
								try {
									...
									if (params.columnParams.listTypeData.name == 'liveaction') {
										const providerType = result.media_type
											? result.media_type
											: params.columnParams.selectedSearchType
										...
										addRow = {
											...
											position: params.params.data.position,
										...
										addRow = {
											/*id: " ", */ mediaIdentity: mediaIdentityForMal(
												result.id,
												'anime',
											),
											mediaRelations: resultInfo.mediaRelations,
											...
											encodeURIComponent(
												new URLSearchParams({
													...
													listTypeData: JSON.stringify(
														params.columnParams.listTypeData,
													),
									...
									if (!addResponse.ok) throw new Error(await addResponse.text())

									const cleanupResponse = await fetch(
										'/lists/fetch/delete-empty-rows/' +
											encodeURIComponent(
												new URLSearchParams({
													authorization: params.columnParams.VEUD_API_KEY,
													watchlistId: params.params.data.watchlistId,
									...
									console.error(
										'[watchlist] failed to add search result',
										error,
									)
									setErrorMessage('Could not add this entry. Please try again.')
								} finally {
									setShowDropdown(true)
								}
							}}
				...
				<div role="status">
					<svg
						aria-hidden="true"
						className="inline h-8 w-8 animate-spin fill-[#FF9900] text-gray-200 dark:text-gray-600"
						viewBox="0 0 100 101"
						fill="none"
