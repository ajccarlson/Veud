					...
					{mediaResults.map( result =>
						<div className="watchlist-search-item" onClick={async () => {
							...
							if (params.columnParams.listTypeData.name == "liveaction") {
								...
								addRow = {/*id: " ", */watchlistId: params.params.data.watchlistId, position: params.params.data.position, thumbnail: resultInfo.thumbnail, title: resultInfo.title, type: resultInfo.type, airYear: String(resultInfo.year), length: resultInfo.length, rating: resultInfo.rating, history: JSON.stringify({added: null, started: null, finished: null, progress: null, lastUpdated: null, }), genres: resultInfo.genres , language: resultInfo.language, story: 0, character: 0, presentation: 0, sound: 0, performance: 0, enjoyment: 0, averaged: 0, personal: 0, differencePersonal: 0, tmdbScore: resultInfo.score, differenceObjective: 0, description: resultInfo.description}
							}
							else if (params.columnParams.listTypeData.name == "anime") {
								resultInfo = await getAnimeInfo(result.id)
								addRow = {/*id: " ", */watchlistId: params.params.data.watchlistId, position: params.params.data.position, thumbnail: resultInfo.thumbnail, title: resultInfo.title, type: resultInfo.type, startSeason: resultInfo.startSeason.name, length: resultInfo.length, rating: resultInfo.rating, history: JSON.stringify({added: null, started: null, finished: null, progress: null, lastUpdated: null, }), genres: resultInfo.genres , studios: JSON.stringify(resultInfo.studios), priority: "Low", story: 0, character: 0, presentation: 0, sound: 0, performance: 0, enjoyment: 0, averaged: 0, personal: 0, differencePersonal: 0, malScore: resultInfo.malScore, differenceObjective: 0, description: resultInfo.description}
							}
							else if (params.columnParams.listTypeData.name == "manga") {
								resultInfo = await getMangaInfo(result.id)
								addRow = {/*id: " ", */watchlistId: params.params.data.watchlistId, position: params.params.data.position, thumbnail: resultInfo.thumbnail, title: resultInfo.title, type: resultInfo.type, startYear: String(resultInfo.startYear), chapters: String(resultInfo.chapters), volumes: String(resultInfo.volumes), history: JSON.stringify({added: null, started: null, finished: null, progress: null, lastUpdated: null, }), genres: resultInfo.genres , serialization: JSON.stringify(resultInfo.serialization), authors: JSON.stringify(resultInfo.authors), priority: "Low", story: 0, character: 0, presentation: 0, enjoyment: 0, averaged: 0, personal: 0, differencePersonal: 0, malScore: resultInfo.malScore, differenceObjective: 0, description: resultInfo.description}
							} 
