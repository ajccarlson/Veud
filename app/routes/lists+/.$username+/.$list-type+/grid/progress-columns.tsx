	...
	return [
		{
			...
			cellRenderer: (params: any) => {
				const totalLength = String(params.value ?? '').trim()
          ...
          return ""
					...
					return totalLength
				} else if (totalLength.toLowerCase().includes('eps')) {
							...
							{columnParams.currentUserId == columnParams.listOwner.id ? (
								<Form
									...
									onSubmit={async (event: any) => {
										...
										const newParams = {
											...params,
											newValue: event.target.lengthInput.value,
								...
								>
									...
									<button
										type="button"
										className="ag-progress-increment-button"
										aria-label={`Increase ${params.data.title || 'entry'} episode progress`}
										onClick={() => {
											const newParams = {
												...params,
												newValue: lengthData.progress + 1,
												oldValue: params.value,
											}
											setterFunction(newParams)
										}}
...

		{
			...
			headerName: 'Chapters',
			cellRenderer: (params: any) => {
				const chapterData = mediaProgressParser(
					params,
					columnParams,
					params.value,
					undefined,
				) as any
				const hasTitle = String(params.data.title ?? '').replace(/\W/g, '')
				const hasType = String(params.data.type ?? '').replace(/\W/g, '')
				const emptyCell = !(hasTitle && hasType)

				return (
					<div className="ag-progress-cell">
						{emptyCell ? (
							<div className="ag-progress-cell-text-container"></div>
						) : columnParams.currentUserId == columnParams.listOwner.id &&
						  hasTitle &&
						  hasType ? (
							<Form
								...
								onSubmit={async (event: any) => {
									...
									const newParams = {
										...params,
										newValue: event.target.chapterInput.value,
										oldValue: params.value,
									}
							...
							>
								...
								<button
									type="button"
									className="ag-progress-increment-button"
									aria-label={`Increase ${params.data.title || 'entry'} chapter progress`}
									onClick={() => {
										const newParams = {
											...params,
											newValue: chapterData.progress + 1,
											oldValue: params.value,
										}
										setterFunction(newParams)
									}}
...

		{
			...
			headerName: 'Volumes',
			cellRenderer: (params: any) => {
				const volumeData = mediaProgressParser(
					params,
					columnParams,
					params.value,
					undefined,
						  ...
						  !emptyCell ? (
							<Form
								...
								onSubmit={async (event: any) => {
									...
									const newParams = {
										...params,
										newValue: event.target.volumeInput.value,
										oldValue: params.value,
									}
							...
							>
								...
								<button
									type="button"
									className="ag-progress-increment-button"
									aria-label={`Increase ${params.data.title || 'entry'} volume progress`}
									onClick={() => {
										const newParams = {
											...params,
											newValue: volumeData.progress + 1,
											oldValue: params.value,
										}
										setterFunction(newParams)
									}}
