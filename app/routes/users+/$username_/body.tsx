		...
		<div className="user-landing-recent-activity-container">
			...
			{allActivity.length > 0 ? (
						...
						<ul className="user-landing-feed">
							{visible.map(item => {
								const thumb = item.thumbnail
									? getThumbnailInfo(item.thumbnail)
									: null
								return (
									<li className="user-landing-feed-item" key={item.key}>
										...
										<div className="user-landing-feed-body">
											{item.title ? (
												<span className="user-landing-feed-title">
													{item.title}
												</span>
											) : null}
											<span className="user-landing-feed-action">
												{item.action}
											</span>
										</div>
				...
				<ProfileEmptyState
					...
					description="Ratings, progress, reviews, and diary entries will appear here."
				/>
			)}
		</div>
	)
...
export function FavoritesData({
	...
	const typedFavorites = (loaderData.favorites ?? []).reduce(
		(x: Record<string, FavoriteItem[]>, y) => {
	...
	return (
		<div className="user-landing-favorites-container">
			...
			<div className="user-landing-favorites-controls">
				<TypeSwitcher
					variant="primary"
					options={loaderData.listTypes.map(type => ({
						key: type.id,
						label: type.header,
					}))}
					index={typeIndex}
			...
			{favorites.length > 0 ? (
				<div className="user-landing-favorites-grid">
					{favorites.map((favorite, index) => {
						const thumb = favorite.thumbnail
							? getThumbnailInfo(favorite.thumbnail)
							: null
						return (
							<div className="user-landing-favorite-card" key={favorite.id}>
								<Link
									to={
										favorite.mediaId
											? `/media/${favorite.mediaId}`
											: (thumb?.url ?? '#')
									}
									className="user-landing-favorite-thumbnail"
									style={
										thumb
											? { backgroundImage: `url("${thumb.content}")` }
											: undefined
									}
								>
									<span className="user-landing-favorite-meta">
										<span className="user-landing-favorite-year">
											{favorite.startYear}
										</span>
										<span className="user-landing-favorite-media-type">
											{favorite.mediaType}
										</span>
									</span>
				...
				<ProfileEmptyState
					...
					description={
						isOwner
							? 'Use the add button above to start curating this shelf.'
							: 'This member has not selected any favorites for this media type.'
					}
				/>
			)}
		</div>
