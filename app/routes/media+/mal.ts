export async function searchMAL(entry: string, type = 'anime', numResults = 5) {
	...
	return data.data.map((entry: any) => entry.node).slice(0, numResults)
}

export async function getAnilistSchedule(entryID: any) {
	try {
		let response: any, data: any
      ...
      `
			...
			console.error(
				'Failed to fetch anilist schedule data for ' + entryID + '!\n' + e,
			)
			return
		}
...
async function formatAnimeInfo(data: any, full = true) {
	try {
		...
		if (typeFormatted.length <= 3) typeFormatted = typeFormatted.toUpperCase()
		else {
		...
		if (data['num_episodes'] == 1) {
			...
			if (typeFormatted == 'TV') typeFormatted = 'TV Series'
...
export async function formatMangaInfo(data: any, full = true) {
	try {
		let typeFormatted = data['media_type'].replace('_', ' ')
		if (typeFormatted.length <= 3) typeFormatted = typeFormatted.toUpperCase()
		else {
		...
		let serialization = []
		for (let magazine of data['serialization'].map(
			(entry: any) => entry.node,
		)) {
			serialization.push(
				magazine['name'] +
...
export async function getMangaInfo(entryID: any) {
	const url =
...
export async function getTopEntries(
	...
	try {
		...
		for (const result of dataResults.values()) {
			...
			else resultArray.push(await formatMangaInfo(result, false))
		}
...
function getSeasonMonth(month: number) {
...
export async function getSeasonalAnime(
	...
	try {
		...
		} else if (isNaN(new Date(`${monthNum} 1, ${yearNum}`).getTime())) {
			const currentDate = getCurrentDate()
			yearNum = currentDate.yearNum
			monthNum = currentDate.monthNum
		}
		...
		const url =
			...
			'?fields=' +
			fields +
			'&sort=anime_num_list_users&limit=' +
			numResults
		let response: any, data: any
		...
		try {
			...
			console.error(
				'Failed to fetch data for ' + seasonalMonth + ' ' + year + ' !\n' + e,
			)
			return
		}
