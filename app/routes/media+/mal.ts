  ...
  try {
    ...
    if (data.nextAiringEpisode) {
      if (data.streamingEpisodes) {
        const foundEpisode = data.streamingEpisodes.find((episode: any) => episode.title.includes(`Episode ${data.nextAiringEpisode.episode} - `))
...
export async function formatMangaInfo(data: any, full = true) {
  try {
    ...
    let serialization = []
    for (let magazine of data['serialization'].map((entry: any) => entry.node)) {
...
export async function getTopEntries(medium = "anime", topType = "all", numResults = 10) {
  try {
    ...
    const dataResults = data.data.map((entry: any) => entry.node).slice(0, numResults)
...
export async function getSeasonalAnime(year: any = undefined, month: any = undefined, numResults = 10) {
  try {
    let yearNum = year
    let monthNum = month
