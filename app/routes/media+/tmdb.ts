...
async function getTMDBCollectionRelations(
	collectionID: any,
	currentEntryID: any,
) {
...
export async function formatTMDBResults(
	entry: any,
	type: string,
	entryID: any,
	dataPass: any,
	...
	try {
		...
		if (!posterFill) posterFill = data.profile_path
	...
	try {
		...
		if (!isNaN(entry)) entryID = entry
			...
			if (entry.match(imdbRegex)) {
				...
				if (type == 'tv') entryID = data.tv_results[0].id
				else if (type == 'person') entryID = data.person_results[0].id
				else entryID = data.movie_results[0].id
			} else {
				url =
					'https://api.themoviedb.org/3/search/' +
					type +
				...
				if (type == 'multi') type = data.results[0].media_type
			}
		}

		url =
			'https://api.themoviedb.org/3/' + type + '/' + entryID + '?language=en-US'
		...
		if (type == 'movie') {
			mediaRelations = data.belongs_to_collection?.id
				? await getTMDBCollectionRelations(
...
export async function getTMDBTrending(type: string, numResults?: number) {
	try {
		...
		const url =
			...
			type +
			'/day?language=en-US&without_keywords=161919'
		let response: any, data: any
