...
export function mediaProgressParser(params: any, columnParams: any, oldValue: any, newValue: any) {
  let mediaType: any

  try {
    const mediaTypeArray = JSON.parse(columnParams.listTypeData.mediaType) as any[]
    ...
    if (newValue) {
      ...
      try {
        ...
        Object.entries(progressObject).forEach(([progressKey, progressValue]: [string, any]) => {
...
export function getStartYear(entry: any, passedType: any, listTypes: any[]) {
  try {
    ...
    if (Object.keys(JSON.parse(typeData.columns) as Record<string, unknown>).includes("airYear")) {
      return entry.airYear
    }
    else if (Object.keys(JSON.parse(typeData.columns) as Record<string, unknown>).includes("startSeason")) {
      return entry.startSeason
    }
    else if (Object.keys(JSON.parse(typeData.columns) as Record<string, unknown>).includes("startYear")) {
...
export function getSiteID(url: string) {
  try {
    ...
    if (linkSplit.findIndex(element => element.includes("imdb")) > -1)
      linkSite = 'imdb';
    else if (linkSplit.findIndex(element => element.includes("tmdb")) > -1 || linkSplit.findIndex(element => element.includes("themoviedb")) > -1)
      linkSite = 'tmdb';
