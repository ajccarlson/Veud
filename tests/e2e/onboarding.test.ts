...
test('onboarding with link', async ({ page, getOnboardingData }) => {
	...
	await expect(
		page.getByRole('button', { name: /continue/i, disabled: true }),
	).toBeVisible()
	...
	expect(email.from).toBe('onboarding@veud.net')
	...
	await expect(
		page.getByText(onboardingData.name, { exact: true }),
	).toHaveCount(0)
...
test('login as existing user', async ({ page, insertNewUser }) => {
	...
	invariant(user.username, 'User name not found')
...
test('reset password with a link', async ({ page, insertNewUser }) => {
	...
	await expect(
		page.getByRole('button', { name: /send reset code/i, disabled: true }),
	).toBeVisible()
	await expect(page.getByText(/check your email/i)).toBeVisible()
